...
def trust_judgement(trust, N):
    for i in range(1, N + 1):
        print(list(filter(lambda x: x[0] == i, trust)))
        # print(list(filter(lambda x: x[1] == i, trust)))

        # 만약에 i에 (1,2,3) 해당하는 사람중에 어느 한명이 믿는 사람이 없으면 실행하지 않는다.
        # 예를 들어 3번이 어느 누구도 판사라고 믿지 않는다면 코드가 실행되지 않고 다음으로 넘어간다.
        ...
        # 여기로 넘어오면 i를 믿는 사람b의 수가 전체 인원-1명이 i를 판사라고 믿는지 확인한다.
